...
class _ChatWidgetState extends State<ChatWidget> {
  ...
  Map<String, dynamic> get _healthContext {
    ...
    final steps = stepsState is StepSuccessState
        ? stepsState.stepModel.fold<double>(0, (s, e) => s + (e.value?.numericValue ?? 0)).toInt()
        : 0;

    // Water (liters → ml)
    final waterState = context.read<WaterCubit>().state;
    final water = waterState is WaterSuccessState
        ? (waterState.waterModel.fold<double>(0, (s, e) => s + (e.value?.numericValue ?? 0)) * 1000).toInt()
        : 0;

    // Nutrition
    final nutritionState = context.read<NutritionCubit>().state;
    final nutritionItems = nutritionState is NutritionSuccess ? nutritionState.nutritionModel : [];
    final calories = nutritionItems.fold<double>(0, (s, e) => s + (e.value?.calories ?? 0));
    final protein = nutritionItems.fold<double>(0, (s, e) => s + (e.value?.protein ?? 0));
    final carbs = nutritionItems.fold<double>(0, (s, e) => s + (e.value?.carbs ?? 0));
    final fat = nutritionItems.fold<double>(0, (s, e) => s + (e.value?.fat ?? 0));

    // Sleep (minutes → hours)
    final sleepState = context.read<SleepCubit>().state;
    final sleep = sleepState is SleepSuccessState
        ? double.parse(((sleepState.sleepModel.fold<double>(0, (s, e) => s + (e.value?.numericValue ?? 0))) / 60).toStringAsFixed(1))
        : 0.0;

    // Weight
    final weightState = context.read<WeightCubit>().state;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(CupertinoIcons.clock, color: _textSecondary, size: 20),
            onPressed: () => _scaffoldKey.currentState?.openDrawer(),
          ),
          IconButton(
            icon: const Icon(CupertinoIcons.square_pencil, color: _textSecondary, size: 20),
                ...
                if (state is ChatUpdated) {
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      if (index == state.messages.length) return _buildTypingIndicator();
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(CupertinoIcons.chat_bubble_2, size: 64, color: Colors.grey.shade300),
      ...
      child: Container(
        ...
        child: MarkdownBody(
          ...
          styleSheet: MarkdownStyleSheet(
            p: TextStyle(color: isUser ? Colors.white : _textPrimary, fontSize: 14, height: 1.4),
            strong: TextStyle(color: isUser ? Colors.white : _textPrimary, fontWeight: FontWeight.bold),
            em: TextStyle(color: isUser ? Colors.white : _textPrimary, fontStyle: FontStyle.italic),
            listBullet: TextStyle(color: isUser ? Colors.white : _textPrimary, fontSize: 14),
          ),
        ),
      ),
    );
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const Spacer(),
                  IconButton(
                    icon: const Icon(CupertinoIcons.xmark, size: 18, color: _textSecondary),
            ...
            Expanded(
              child: sessions.isEmpty
                ...
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(CupertinoIcons.clock, size: 40, color: Colors.grey.shade300),
