...
class FoodDetailCubit extends Cubit<FoodDetailState> {
  ...
  void fetchFoodDetails(String foodId) async {
    ...
    final baseString = sortedParams
        .map((e) =>
            '${Uri.encodeComponent(e.key)}=${Uri.encodeComponent(e.value)}')
        .join('&');
    final signatureBaseString =
        'GET&${Uri.encodeComponent(url)}&${Uri.encodeComponent(baseString)}';

    // Step 3: Generate the HMAC-SHA1 signature
    final signingKey =
        '${Uri.encodeComponent(FAT_SECRET_CONSUMER_SECRET)}&'; // No token secret for 2-legged OAuth
    ...
    final queryString = oauthParams.entries
        .map((e) =>
            '${Uri.encodeComponent(e.key)}=${Uri.encodeComponent(e.value)}')
        .join('&');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        FoodWithServingsModel foodServingModel =
            FoodWithServingsModel.fromJson(data);
